// im-select-jp.cpp : このファイルには 'main' 関数が含まれています。プログラム実行の開始と終了がそこで行われます。
//
...
int main(int argn, char* argv)
{
	...
	HWND hIMEWnd = ImmGetDefaultIMEWnd(hActiveWindow);
	if (!hIMEWnd)
	{
		std::cout << "ImmGetDefaultIMEWnd failed" << std::endl;
		return 1;
	}
